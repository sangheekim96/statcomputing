...
#gradient boosting
GBResults <- function(measures, nfold = 10, ntrees = 1000, featuresBase = c("Roll", "Roll.Impact", "Kyle", "Amihud", "VPIN"),
  ...
  #accList <- list() # to store accuracies
    ...
    for(es in sizes) { # loop over sizes to predict (e.g., Kurtosis.Big)
      ...
      for(et in types) { # loop over whether to include cross-effects
        ...
        for(i in 1:nfold) { # loop over folds
          ...
          ytrain <- train[,lab]
          gb <- gbm.fit(x=train[,features], y=ytrain, distribution="multinomial", shrinkage=0.001, 
                        bag.fraction = 0.5, n.minobsinnode = 10, n.trees=ntrees, verbose = FALSE)
          xtest <- test[,features]
          ytest <- test[,lab]
          predictions <- as.data.frame(predict(gb, xtest, type="response"))
          predictions <- rename(predictions, c('-1.1000'='-1', '0.1000'='0', '1.1000'='1'))
