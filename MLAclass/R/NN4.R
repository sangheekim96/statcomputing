...
NN4result <- function(measures, nfold = 10, featuresBase = c("Roll", "Roll.Impact", "Kyle", "Amihud", "VPIN"),
    ...
    for(es in sizes) { # loop over sizes to predict (e.g., Kurtosis.Big)
      ...
      for(et in types) { # loop over whether to include cross-effects
        ...
        for(i in 1:nfold) { # loop over folds
          ...
          lay <- ((ncol(subs)-1)/3)^(1/5)
          nn4 <- neuralnet(ytrain~., data=subs, hidden=c(floor(3*lay^4), floor(3*lay^3), floor(3*lay^2), floor(3*lay)), threshold = 0.5, linear.output = FALSE)
          xtest <- test[,features]
          ytest <- test[,lab]
          predictions <- as.data.frame(predict(nn4, xtest, type="prob"))
